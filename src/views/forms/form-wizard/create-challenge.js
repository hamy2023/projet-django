...
const StepperAlternativeLabel = () => {
  ...
  const handleDraft = async e => {
    ...
    try {
      ...
      const draftResponse = await fetch('http://localhost:8000/challenge-management/drafts/create', {
        method: 'POST',
        body: formData
      })
      ...
      const draftIdResponse = await fetch(`http://localhost:8000/challenge-management/drafts/${name}/get_id/`)
      const draftIdData = await draftIdResponse.json()
      console.log(draftIdData)

      // Third API call to create the questions
      const draftquestionData = await Promise.all(
        tasks.map(async task => {
          const response = await fetch('http://localhost:8000/challenge-management/draftquestions/create', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
      ...
      const draftsolutionResponse = await fetch('http://localhost:8000/challenge-management/draftsolutions/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      toast.success('Challenge Saved as Draft!')
    } catch (error) {
      console.error(error)
  ...
  const handleTaskChange = (id, description, points, vr, editor, bank) => {
    const updatedTasks = tasks.map(task => {
      if (task.id === id) {
        return { ...task, description, points, vr, editor, bank }
      }
      return task
    })
    setTasks(updatedTasks)
              ...
              <Grid container item spacing={2} key={task.id}>
                ...
                <Grid item xs={2}>
                  <TextField
                    ...
                    label='Points'
                    value={task.points}
                    onChange={e => handleTaskChange(task.id, task.description, e.target.value, task.vr, task.editor)}
                  />
                  <FormControl fullWidth>
                    ...
                    <Select
                      value={task.bank}
                      onChange={e =>
                        handleTaskChange(task.id, task.description, task.points, task.vr, task.editor, e.target.value)
                      }
                    >
                      {banks.map(bank => (
            ...
            <Grid item xs={12}>
              ...
              >
                ...
                <Editor
                  language='python'
                  theme='vs-dark'
                  defaultValue='#challenge solution here'
